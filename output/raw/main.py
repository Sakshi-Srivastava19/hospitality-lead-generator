...
if os.path.exists(all_file):
    ...
    # Guard against stale rows collected before PRICE_FILTER_MIN/MAX was
    # working correctly (or from a run with a different filter) leaking
    # into today's band CSVs. OUTPUT_ALL_FILE is intentionally left as the
    # full historical record; only the per-band exports are restricted.
    if PRICE_FILTER_MIN is not None:
        before = len(priced)
        priced = priced[priced["PRICE_NUM"] >= PRICE_FILTER_MIN]
        dropped_min = before - len(priced)
    else:
        dropped_min = 0
    if PRICE_FILTER_MAX is not None:
        before = len(priced)
        priced = priced[priced["PRICE_NUM"] <= PRICE_FILTER_MAX]
        dropped_max = before - len(priced)
    else:
        dropped_max = 0
    if dropped_min or dropped_max:
        print(f"  Excluded {dropped_min + dropped_max} leads outside "
              f"PRICE_FILTER_MIN/MAX (₹{PRICE_FILTER_MIN}-₹{PRICE_FILTER_MAX}) "
              f"from band CSVs -- still present in {all_file}.")

    no_price = len(full_df) - len(full_df.dropna(subset=["PRICE_NUM"]))
